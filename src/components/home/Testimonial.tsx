      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5 auto-rows-auto">
          ...
          <div className="relative p-8 lg:p-10 rounded-[var(--radius-xl)] bg-[var(--primary-black)] border border-[var(--primary-black)] transition-all duration-[var(--transition-base)] hover:-translate-y-1 hover:shadow-2xl overflow-hidden lg:row-span-2 flex flex-col justify-between animate-fade-in-up group">
            <div className="absolute top-[-20px] right-4 text-[140px] font-black text-[var(--primary-white)]/5 leading-none font-oswald pointer-events-none select-none">&ldquo;</div>
            <div>
              ...
              <p className="type-body text-[var(--primary-white)]/90 text-lg leading-relaxed relative z-10 mb-8">
                &ldquo;After just 3 sessions with CueClarity, I had a crystal-clear roadmap to transition from software engineering into product management. The process was incredibly personalized — they actually listened to what mattered to me, not just my resume. I landed my dream PM role within 4 months.&rdquo;
              </p>
            </div>
            <div className="flex items-center gap-3 relative z-10 border-t border-[var(--primary-white)]/10 pt-6 mt-auto">
              <div className="w-10 h-10 rounded-[var(--radius-md)] bg-[var(--primary-blue)] text-[var(--primary-white)] flex items-center justify-center font-bold text-lg">A</div>
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[var(--primary-white)]">Ananya Mehta</span>
                <span className="text-[11px] text-[var(--primary-white)]/50 mt-0.5">Software Engineer → PM · Bangalore</span>
              </div>
            </div>
          </div>
          ...
          >
            <div className="absolute top-[-15px] right-3 text-[100px] font-black text-[var(--primary-white)]/10 leading-none font-oswald pointer-events-none select-none">&ldquo;</div>
            <div>
              <div className="text-[var(--primary-white)]/70 text-[13px] tracking-widest mb-3">★★★★★</div>
              <p className="type-body text-[var(--primary-white)]/90 text-[14px] relative z-10 mb-6">
                &ldquo;Switched careers after 8 years. CueClarity made it feel possible, not terrifying. Best decision I ever made.&rdquo;
              </p>
            </div>
            <div className="flex items-center gap-3 relative z-10">
              <div className="w-10 h-10 rounded-[var(--radius-sm)] bg-[var(--primary-white)]/20 text-[var(--primary-white)] flex items-center justify-center font-bold text-[15px]">R</div>
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[var(--primary-white)]">Rahul Desai</span>
                <span className="text-[11px] text-[var(--primary-white)]/60 mt-0.5">Finance → Data Science</span>
              </div>
            </div>
          </div>
...
function TestimonialCard({ quote, initial, avatarBg, name, role, delay, bgColor = "bg-white" }: {
  quote: string;
  initial: string;
  avatarBg: string;
  name: string;
  ...
  return (
    ...
    >
      <div className="absolute top-[-15px] right-3 text-[100px] font-black text-[var(--primary-black)]/5 leading-none font-oswald pointer-events-none select-none">&ldquo;</div>
      <div>
        <div className="text-[var(--primary-yellow)] text-[13px] tracking-widest mb-3">★★★★★</div>
        <p className="type-body text-[var(--primary-black)] text-[14px] relative z-10 mb-6">
          &ldquo;{quote}&rdquo;
        </p>
      </div>
      <div className="flex items-center gap-3 relative z-10">
        <div className={`w-10 h-10 rounded-[var(--radius-sm)] ${avatarBg} text-white flex items-center justify-center font-bold text-[15px]`}>
          {initial}
        </div>
        <div className="flex flex-col">
          <span className="text-[13px] font-bold text-[var(--primary-black)] leading-tight">{name}</span>
          <span className="text-[11px] text-[var(--muted-text)] mt-0.5">{role}</span>
        </div>
      </div>
    </div>
  );
