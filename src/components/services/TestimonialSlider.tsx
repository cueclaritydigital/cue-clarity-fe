...
interface Testimonial { _key: string; quote: string; name: string; role?: string; }
...
const avatarColors = [
  "var(--primary-blue)", "var(--primary-yellow)", "var(--accent-blue)",
  "#9333ea", "#ea580c", "#16a34a",
];

export default function TestimonialSlider({ data }: { data: TestimonialSliderData }) {
    ...
    <section className="section-padding overflow-hidden" style={{ backgroundColor: BG[color] }}>
      <div className="section-container">
        ...
        <div className={`grid gap-5 ${
          data.testimonials?.length === 1 ? "grid-cols-1 max-w-2xl" :
          data.testimonials?.length === 2 ? "grid-cols-1 sm:grid-cols-2" :
            ...
            <FadeUp key={t._key} delay={i * 0.07}>
              <div
                ...
                style={{
                  ...
                  border: isDark ? "1px solid rgba(255,255,255,0.1)" : "1px solid var(--secondary-white)",
                  boxShadow: "var(--shadow-sm)",
                }}
              >
                <div aria-hidden className="absolute top-[-10px] right-3 font-oswald font-black pointer-events-none select-none leading-none"
                  style={{ fontSize: "100px", color: isDark ? "rgba(255,255,255,0.04)" : "rgba(12,21,24,0.04)" }}>
                  &ldquo;
                </div>
                <div>
                  <div className="text-[var(--primary-yellow)] text-sm tracking-widest mb-4">★★★★★</div>
                  <p className="text-sm leading-relaxed relative z-10"
                    style={{ color: isDark ? "rgba(255,255,255,0.85)" : "var(--primary-black)" }}>
                    &ldquo;{t.quote}&rdquo;
                  </p>
                </div>
                <div className="flex items-center gap-3 border-t pt-5"
                  style={{ borderColor: isDark ? "rgba(255,255,255,0.1)" : "var(--secondary-white)" }}>
                  ...
                  <div>
                    ...
                    {t.role && (
                      <p className="text-xs mt-0.5" style={{ color: isDark ? "rgba(255,255,255,0.5)" : "var(--muted-text)" }}>
                        {t.role}
                      </p>
                    )}
                  </div>
